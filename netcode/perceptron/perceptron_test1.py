...
class Perceptron:
    ...
    def __init__(self, name=None):
        ...
        self.transfer = self.sigmoid_transfer
        #partial derivative of logistic curve
        self.d_transfer = self.sigmoid_transfer_derivative
        #error function
    ...
    def sigmoid_transfer_derivative(x):
        sigmoid_x = Perceptron.sigmoid_transfer(x)
    ...
    def get_output(self):
        if self.input is None:
            x = 0
            for node, info in self.inputs.items():
                x = x + node.get_output() * info["weight"] * info["attention"]
            # pp("bias: " + str(self.bias) + " x: " + str(x))
    ...
    def _train_internal(self, delta_prev):
        ...
        # self.bias = self.bias - delta_prev (why did I have this here and why was it wrong?)
        self.bias = self.bias - self.learning_rate * self.d_transfer(self.get_output()) * delta_prev
        ...
        for node, info in self.inputs.items():
            ...
            self.inputs[node]["attention"] = info["attention"] - self.learning_rate * delta_prev * node_output
    ...
    def init_synapse(self, p):
        if p not in self.inputs:
            self.inputs[p] = {"weight": random.uniform(-1,1), "attention": 1}
        else:
            self.inputs[p]["weight"] = random.uniform(-1, 1)
            self.inputs[p]["attention"] = 1
        while not self.inputs[p]["weight"]:
            self.inputs[p]["weight"] = random.uniform(-1,1)
        while not self.inputs[p]["attention"]:
            self.inputs[p]["attention"] = 1
